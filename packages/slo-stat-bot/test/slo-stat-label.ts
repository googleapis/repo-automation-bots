  ...
  describe('Checking for compliant settings given a slo applies to an issue', () => {
    ...
    describe('repo level file exists with write contributer being valid responder', () => {
     ...
     const writeConfigContent = require(resolve(
      ...
      it('Returns true if issue is within resolution time, a valid responder was assigned, a valid responder commented within response time', async () => {
        const requests = nock('https://api.github.com')
          ...
          .reply(200, [{login: "user3", permissions:{admin: false, push: true, pull: true}}])
          .get('/repos/testOwner/testRepo/issues/5/comments')
          .reply(200, [{user:{login: "user1"}, created_at: "2020-07-22T03:04:47Z"}])
      ...
      it('Returns false if no valid responder was assigned', async () => {
        const requests = nock('https://api.github.com')
          .get('/repos/testOwner/testRepo/contents/.github/issue_slo_rules.json')
          .reply(200, writeConfigContent)
          .get('/repos/testOwner/testRepo/collaborators')
          .reply(200, [{login: "customer1", permissions:{admin: false, push: true, pull: true}}])
      ...
      it('Returns false if no valid responder commented', async () => {
        const requests = nock('https://api.github.com')
          ...
          .reply(200, [{login: "user3", permissions:{admin: false, push: true, pull: true}}])
          .get('/repos/testOwner/testRepo/issues/5/comments')
          .reply(200, [{user:{login: "customer1"}, created_at: "2020-06-02T16:00:49Z"}])
      ...
      it('Returns false if valid responder commented past duration time', async () => {
        const requests = nock('https://api.github.com')
          .get('/repos/testOwner/testRepo/contents/.github/issue_slo_rules.json')
          .reply(200, writeConfigContent)
          .get('/repos/testOwner/testRepo/collaborators')
          .reply(200, [{login: "user3", permissions:{admin: false, push: true, pull: true}}])
          .get('/repos/testOwner/testRepo/issues/5/comments')
          .reply(200, [{user:{login: "user3"}, created_at: "2020-07-28T03:04:47Z"}])
    ...
    describe('org level file exists with admin contributer being valid responder', () => {
      ...
      it('Returns true if issue is within resolution time, a valid responder was assigned, a valid responder commented within response time', async () => {
        const requests = nock('https://api.github.com')
          ...
          .get('/repos/testOwner/testRepo/collaborators')
          .reply(200, [{login: "writer1", permissions:{admin: false, push: true, pull: true}},
                       {login: "admin1", permissions:{admin: true, push: true, pull: true}}])
          .get('/repos/testOwner/testRepo/issues/5/comments')
          .reply(200, [{user:{login: "admin1"}, created_at: "2020-07-22T13:04:47Z"}])
      ...
      it('Returns false if issue does not have comment from valid responder', async () => {
        const requests = nock('https://api.github.com')
          ...
          .reply(200, [{login: "writer1", permissions:{admin: false, push: true, pull: true}},
                       {login: "admin1", permissions:{admin: true, push: true, pull: true}}])
          .get('/repos/testOwner/testRepo/issues/5/comments')
          .reply(200, [{user:{login: "writer1"}, created_at: "2020-07-22T13:04:47Z"}])
    ...
    describe('repo level file exists with owner contributer being valid responder', () => {
      ...
      it('Returns true if issue is within resolution time, a valid responder commented within response time', async () => {
        const requests = nock('https://api.github.com')
          ...
          .reply(200, [])
          .get('/repos/testOwner/testRepo/issues/5/comments')
          .reply(200, [{user:{login: "testOwner"}, created_at: "2020-07-22T08:03:47Z"}])
    ...
    describe('repo level file exists with owners path containing valid responder', () => {
      ...
      it('Returns true if issue is within resolution time, assigned to valid responder, a valid responder commented within response time', async () => {
        const requests = nock('https://api.github.com')
          ...
          .get('/repos/testOwner/testRepo/issues/5/comments')
          .reply(200, [{user:{login: "Coder-cat."}, created_at: "2020-07-22T03:05:47Z"}])
