  ...
  describe('findFailures', () => {
    it('finds one failure', () => {
      const input = fs.readFileSync(
        resolve(fixturesPath, 'testdata', 'one_failed.xml'), 'utf8'
      );
      const failures = findFailures(input);
      expect(failures).to.eql([{
        package: "github.com/GoogleCloudPlatform/golang-samples/spanner/spanner_snippets",
        testCase: "TestSample"
      }]);
    });
    ...
    it('finds many failures in one package', () => {
      const input = fs.readFileSync(
        resolve(fixturesPath, 'testdata', 'many_failed_same_pkg.xml'), 'utf8'
      );
      const failures = findFailures(input);
      expect(failures).to.eql([
        {
          "package": "github.com/GoogleCloudPlatform/golang-samples/storage/buckets",
    ...
    it('finds no failures in a successful log', () => {
      const input = fs.readFileSync(
        resolve(fixturesPath, 'testdata', 'passed.xml'), 'utf8'
      );
      const failures = findFailures(input);
      expect(failures).to.eql([]);
    });
    ...
    it('opens an issue', async () => {
      ...
      var payload = require(resolve(fixturesPath, './events/issue_unlabeled'));
      payload.xunit_xml = input;
