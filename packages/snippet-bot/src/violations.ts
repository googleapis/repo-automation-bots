...
import {ApiLabels} from './api-labels';
...
export const checkRemovingUsedTagViolations = async (
  ...
  const snippets = await getSnippets(dataBucket, logger);
  for (const change of changes.changes) {
    if (change.type !== 'del') {
      continue;
    }
...
export const checkProductPrefixViolations = async (
  ...
  for (const change of changes.changes) {
    ...
    if (isAutoGenerated(change.regionTag)) {
      if (
        !apiLabels.products.some(apiLabel => {
          return change.regionTag.startsWith(apiLabel.api_shortname + '_');
        })
      ) {
        ret.push({
          location: change,
      ...
      if (
        !apiLabels.products.some(apiLabel => {
          return change.regionTag.startsWith(apiLabel.region_tag_prefix + '_');
        })
      ) {
        ret.push({
          location: change,
