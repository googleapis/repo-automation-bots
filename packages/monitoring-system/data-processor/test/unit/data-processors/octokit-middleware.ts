...
export class OctokitMiddleware {
    ...
    public static getInstance(): OctokitMiddleware {
        if (!this.instance) {
    ...
    public getMockResponse(options: {
        ...
        const {type, value, destObjId, destObjType, repoName, repoOwner } = this.getStrictDetails(action);
        console.log({type, value, destObjId, destObjType, repoName, repoOwner })
        console.log(this.mockResponses);
        const response = this.mockResponses[type][value][destObjType][destObjId][repoName][repoOwner]
    ...
    public rejectOnAction(action: GitHubActionDetails) {
        this.setMockResponse(action, { type: 'reject', value: { 'Error': 'A mock error'}});
    }

    private getStrictDetails(action: GitHubActionDetails): GitHubActionDetailsStrict {
        return {
