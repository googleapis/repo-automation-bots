...
import {addLabels} from '../../src/code-suggester/github/labels';

import responseAddLabelsData from '../fixtures/add-labels-response.json';
...
describe('Adding labels', async () => {
  ...
  it('Invokes octokit issues add labels on an existing pull request', async () => {
    ...
    const addLabelsResponse: AddLabelsResponse = {
      ...
      data: responseAddLabelsData as unknown as AddLabelsResponse['data'],
    };
    const stub = sandbox
