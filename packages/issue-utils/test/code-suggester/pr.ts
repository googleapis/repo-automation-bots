...
import {openPullRequest} from '../../src/code-suggester/github/open-pull-request';

import responseCreatePullData from '../fixtures/create-pr-response.json';
import responseListPullData from '../fixtures/list-pulls-response.json';
...
describe('Opening a pull request', async () => {
  ...
  it('Invokes octokit pull create when there is not an existing pull request open', async () => {
    ...
    const createPrResponse: CreatePRResponse = {
      ...
      data: responseCreatePullData as unknown as CreatePRResponseData,
    };
  ...
  describe('When there are similar refs with pull requests open, the current new and unique ref still opens a pr', async () => {
    ...
    const listPullResponse: ListPullsResponse = {
      ...
      status: 200,
      url: 'http://fake-url.com',
      data: responseListPullData as unknown as ListPullsResponseData,
    };
