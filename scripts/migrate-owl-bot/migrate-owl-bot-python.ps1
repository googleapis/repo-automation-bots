...
"
    ...
    try {
        ...
        while ($true) {
            ...
            docker run --user "$(id -u):$(id -g)" --rm -v "${localPath}:/repo" -w /repo `
                gcr.io/repo-automation-bots/owlbot-python:latest
            git -C $localPath add -A
...
function Migrate-All([string]$lang, $workDir) {
    ...
    try {
        ...
        # Because github seems to list them in order from most recently modified, and
        # the most recently modified are hardest to migrate.
        [array]::Reverse($repos)
